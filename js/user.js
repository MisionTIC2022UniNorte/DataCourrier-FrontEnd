...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayEmail(user) {
        ...
        usersRef.doc(user.uid).get().then(snapshot => {
            ...
            let userInfoHtml =`
            <li class="bold waves-effect"><i class="material-icons amber-text text-accent-2 medium">Usuario Verificado</i></li>
            <h5>Bienvenido al mundo de los envíos</h5>
            <li><p class="teal-text">${user.email}</p></li>
            `;
            let userInfoModalHtml = `
            <div id="accountInfoTitle" class="row z-depth-1">
            <h4 class="center-align">Sus Datos Personales</h4>
            </div>
            <p class="teal-text"><span class="grey-text">Nombre: </span><br>${userData.firstName} ${userData.lastName}</p>
            <p class="teal-text"><span class="grey-text">Registrado como: </span><br>${user.email}</p>
            <p class="teal-text"><span class="grey-text">ID único: </span><br>${user.uid}</p>
            `;
            let userWelcomeMessageHtml =`
            <h4>Saludos ${userData.firstName},</h4>
            <p class="grey-text">Para enviar un paquete haga click en el botón "AGREGAR ORDEN", luego llene el formulario. 
            <br>Importante! No podemos proceder con su pedido, sin recibir el pago previamente.
            <br>Asegúrese de enviar el pago de su pedido, mediante transferencia bancaria, en un plazo de 24 horas.</p>
            `;
